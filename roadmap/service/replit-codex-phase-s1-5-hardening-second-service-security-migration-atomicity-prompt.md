...
Do not keep patch-style migrations for the current schema. Fold current schema definitions into the Drizzle baseline instead of keeping repair migrations such as adding refund columns, adding transaction type, adding API client tables, adding missing constraints, or fixing invalid constraints.

`ALTER TABLE` is only acceptable for genuinely future migrations after this cleanup, not for constructing the current baseline.
