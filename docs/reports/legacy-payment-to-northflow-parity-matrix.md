...
| Provider-level cancel/refund contract parity | Providers declare refund/cancel capability through explicit runtime methods. | Implemented. | `StandalonePaymentProvider` exposes optional `refundPayment()` and `cancelPayment()` contracts. |
